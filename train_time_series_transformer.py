...
# (Assumes prepare_data_timeseries_transformer.py already loads and sorts the CSV.)
full_dataset = TimeSeriesImageDataset(df=None)  # In our file, the dataset reads CSV internally
# If your dataset file doesn’t re-read the CSV when df is None,
# you may need to modify the dataset’s __init__ to load df from "farm_data.csv".
